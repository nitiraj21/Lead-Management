...
export default function UpdateLead({ _id, onClose }: UpdateLeadProps){
    ...
    const UpdateLeads = async (e : React.FormEvent<HTMLFormElement>) =>{
            e.preventDefault();
            setIsLoading(true);
            setError("");
    ...
    return <div className="flex justify-center items-center w-screen min-h-screen ">
    <div className=" bg-white border border-2 border-[#E5E7EB] rounded-xl p-10 shadow-xl h-auto w-auto mb-40">
        <form onSubmit={UpdateLeads} className="space-y-5">
            <div> 
                <div className="flex justify-end">
                    <button
                    type="button"
                    onClick={onClose}
                        className="top-2 right-2 text-white bg-red-500 rounded-full w-6 h-6 flex items-center justify-center">
                        X
                    </button>
                </div>
                <h1 className="flex justify-center items-center text-3xl pb-3">Update Lead</h1>
